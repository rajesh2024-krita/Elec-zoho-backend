...
// Update the sales/create endpoint
app.post("/api/sales/create", async (req, res) => {
  try {
    ...
    if (product1) {
      skuFields["SKU1"] = String(sku1 || "");
      modelFields["Model_1"] = String(modelNo1 || "");
      rateFields["Rate_1"] = String(rate1 || "0");
      productNameFields["Product_Name_1"] = String(product1 || "");
    }

    // Additional products from multi-product fields
    if (multiProduct) {
      if (product2) {
        skuFields["SKU2"] = String(sku2 || "");
        modelFields["Model_2"] = String(modelNo2 || "");
        rateFields["Rate_2"] = String(rate2 || "0");
        productNameFields["Product_Name_2"] = String(product2 || "");
      }
      if (product3) {
        skuFields["SKU3"] = String(sku3 || "");
        modelFields["Model_3"] = String(modelNo3 || "");
        rateFields["Rate_3"] = String(rate3 || "0");
        productNameFields["Product_Name_3"] = String(product3 || "");
      }
      if (product4) {
        skuFields["SKU4"] = String(sku4 || "");
        modelFields["Model_4"] = String(modelNo4 || "");
        rateFields["Rate_4"] = String(rate4 || "0");
        productNameFields["Product_Name_4"] = String(product4 || "");
      }
      if (product5) {
        skuFields["SKU5"] = String(sku5 || "");
        modelFields["Model_5"] = String(modelNo5 || "");
        rateFields["Rate_5"] = String(rate5 || "0");
        productNameFields["Product_Name_5"] = String(product5 || "");
      }
    }
    ...
    // Add main product amount
    if (rate1) billAmount += Number(rate1);

    // Add additional product amounts if multi-product is enabled
    if (multiProduct) {
      if (rate2) billAmount += Number(rate2);
      if (rate3) billAmount += Number(rate3);
      if (rate4) billAmount += Number(rate4);
      if (rate5) billAmount += Number(rate5);
    }

    // Prepare product categories array
    const productCategories = [];
    if (products && products.length > 0 && products[0].category) {
      productCategories.push(products[0].category);
    }
      ...
      data: [
        {
          ...
          Multi_Product: multiProduct ? true : false,
          ...
          One_Assist: products?.[0]?.oneAssist || false,
          One_Assist_Amount: products?.[0]?.oneAssistAmount || 0,
          Spin_Percent: products?.[0]?.spinPercent || "",
          Serial_No: products?.[0]?.serialNo || "",
          Installation_Required: products?.[0]?.installationRequired || false,
          Installation_Paid_By: products?.[0]?.installationPaidBy || "",

          GST_INPUT_REQUIRED: gstInputClaim ? true : false,
          GST_Number: gstNumber || "",

          Discount: discount ? true : false,
          How_Much: discountAmount || 0,
          Discount_Approved_By: discountApprover || "",

          Under_Exchange: underExchange ? true : false,
          Under_Exchange_info: exchangeInfo || "",

          Diwali_2024_Spin: diwaliSpinWin ? "Yes" : "No",
          Scheme_Number: schemeNo || "",
          Gift_Contribution: giftAmount || 0,

          Delivery_Later: deliveryLater ? true : false,
    ...
    console.log("Sending payload to Zoho:", JSON.stringify(payload, null, 2));
